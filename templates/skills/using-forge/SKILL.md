...
If developer-agent:
  ...
  → forge_claim_story handles: move to `in-dev` + self-assign in one API call
...
If qa-agent:
  ...
  → forge_claim_story handles: move to `in-qa` + self-assign

If po-agent:
  → Query Linear: oldest story in `ready-for-acceptance`
  → forge_claim_story handles: move to `in-acceptance` + self-assign
```
